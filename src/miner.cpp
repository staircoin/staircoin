...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("STAIRMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("STAIRMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("STAIRMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("stair-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("STAIRMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running STAIRMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadSTAIRMiner() exception");
    } catch (...) {
        LogPrintf("ThreadSTAIRMiner() exception");
    }

    LogPrintf("ThreadSTAIRMiner exiting\n");
}

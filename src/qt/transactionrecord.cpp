        ...
        BOOST_FOREACH (const CTxOut& txout, wtx.vout) {
            ...
            if (mine) {
				...
				if(nHeight <= 1) {
					...
					nSubsidy = 0.4 * COIN;
					if(nSubsidy * 70 / 100 == txout.nValue) {
						sub.type = TransactionRecord::MNReward;
					}
				} else if (nHeight > 4320 && nHeight <= 14401) {
					nSubsidy = 20 * COIN;
					if(nSubsidy * 70 / 100 == txout.nValue) {
						sub.type = TransactionRecord::MNReward;
					}
				} else if (nHeight > 14401 && nHeight <= 24482) {
					nSubsidy = 60 * COIN;
					if(nSubsidy * 70 / 100 == txout.nValue) {
						sub.type = TransactionRecord::MNReward;
					}
				} else if (nHeight > 24482 && nHeight <= 34563) {
					nSubsidy = 120 * COIN;
					if(nSubsidy * 70 / 100 == txout.nValue) {
						sub.type = TransactionRecord::MNReward;
					}
				} else if (nHeight > 34563 && nHeight <= 44644) {
					nSubsidy = 200 * COIN;
					if(nSubsidy * 70 / 100 == txout.nValue) {
						sub.type = TransactionRecord::MNReward;
					}
				} else if (nHeight > 44644 && nHeight <= 1000000) {
					nSubsidy = 300 * COIN;
					if(nSubsidy * 70 / 100 == txout.nValue) {
						sub.type = TransactionRecord::MNReward;
					}
